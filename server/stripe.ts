...
export async function createTokenCheckout(v: { email: string; pack: string; origin: string }): Promise<string> {
  ...
  const s = await stripe("POST", "/checkout/sessions", {
    ...
    "line_items[0][price_data][product_data][name]": `Pōstify · ${pack.tokens} credits`,
    "line_items[0][price_data][product_data][description]": "Credits for clips and clean episodes. Bought once, no subscription. Beta pricing.",
