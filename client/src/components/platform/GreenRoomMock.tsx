...
import { CAST, useFakeCamera } from "./fakeCamera";
...
/** A <video> fed by a fake camera, mirrored as a self-view is. */
function SelfView({ src }: { src: string }) {
  const track = useFakeCamera(src);
  const ref = useRef<HTMLVideoElement | null>(null);
  useEffect(() => {
    const el = ref.current;
    if (!el || !track) return;
    (track as unknown as { attach: (e: HTMLMediaElement) => void }).attach(el);
    return () => (track as unknown as { detach: (e: HTMLMediaElement) => void }).detach(el);
  }, [track]);
  return (
    <>
      <img src={src} alt="" className="absolute inset-0 h-full w-full scale-x-[-1] object-cover" />
      <video ref={ref} autoPlay playsInline muted className="absolute inset-0 h-full w-full scale-x-[-1] object-cover" />
    </>
  );
}
    ...
    <div className="rounded-[1.5rem] bg-white p-5 text-slate-900 shadow-2xl sm:p-6" aria-hidden="true">
      ...
      <div className="relative mt-4 aspect-video overflow-hidden rounded-2xl bg-[#0b1433]">
        ...
        <span className="absolute bottom-2.5 left-2.5 max-w-[80%] truncate rounded-md bg-black/45 px-2 py-0.5 text-xs font-semibold text-white">
          Kim Rowe <span className="font-normal text-white/75">· Squad Bay Radio</span>
        </span>
      </div>
      ...
      <div className="mt-5 grid grid-cols-[1.3fr_1fr] gap-2.5">
        <div>
          ...
          <div className="mt-1 flex h-10 items-center rounded-xl border border-slate-300 px-3 text-sm">Kim Rowe</div>
        </div>
        <div>
          <p className="text-xs font-semibold text-slate-600">Title <span className="font-normal text-slate-400">(optional)</span></p>
          <div className="mt-1 flex h-10 items-center truncate rounded-xl border border-slate-300 px-3 text-sm">Squad Bay Radio</div>
        </div>
      </div>
...
const WAITING = [
  { key: "dana" as const, show: "Homefront Hour", status: "Next up · 4 min", ok: true, cam: true },
  { key: "kim" as const, show: "Squad Bay Radio", status: "Mic and camera checked", ok: true, cam: true },
  { key: "andre" as const, show: "After the Uniform", status: "Camera off", ok: false, cam: false },
];
            ...
            return (
              <li key={w.key} className="flex items-center gap-3 rounded-xl bg-white/[0.04] p-2 ring-1 ring-white/5">
                <div className="relative aspect-video w-24 shrink-0 overflow-hidden rounded-lg bg-[#053877] sm:w-28">
                  {w.cam ? (
                    <img src={p.cam} alt="" loading="lazy" className="h-full w-full object-cover" />
                  ) : (
                    <span className="flex h-full w-full items-center justify-center">
                      <img src={p.face} alt="" className="h-9 w-9 rounded-full object-cover opacity-80 ring-2 ring-white/20" />
                ...
                {w.key === "dana" ? (
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-[#3B82F6] px-2.5 py-1 text-[11px] font-semibold text-white">
                    <ArrowUp className="h-3 w-3" /> On stage
                  </span>
                ) : (
